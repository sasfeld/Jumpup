...
define(
		[ "gmap/googlemap", "jquery", "gmap/overviewPathStrategy" ],
		(function(GoogleMap, $, OverviewPathStrategy) {
			...
			/*
			 * Constructor function for this module. The google map will be
			 * initialized in the constructor. - param options, see the google
			 * map reference - throws an exception if the map isn't loaded
			 */
			 ...
			 * @deprecated
			 */
			MapController.prototype.handleRouteResponse = function(
					directionsResult) {
				...
				if (1 == singleRoute.legs.length) { // no waypoints, only start
					...
					for ( var waypointIndex = 0; waypointIndex < viaWaypoints.length; waypointIndex++) {
						...
						waypointsStringConcat += viaWaypoints[waypointIndex]
								.lat()
			...
			/*
			 * Show a single route on the map. - param start, the value of the
			 * starting point, must be a coordinate or a valid location. - param
			 * destination, the value of the destination point, must be a
			 * coordinate or a valid location. - param waypoints, an array of
			...
			MapController.prototype.showSingleRoute = function(start,
				...
				this.gmap.showRoute(start, destination,
						this.handleRouteResponse);
			};
